...
@media screen and (max-width: 700px) {
  ...
  .topnav.responsive a {
    ...
    text-align: left;
        ...
        <script>
            var busdetails = ${busdata};
            angular.module('myapp', []).controller(
                    'BusController', function ($scope) {
                        $scope.pdo = busdetails;
                    });
        </script>
  </head>
  <body>
	...
	<div class="container">
            <div class="row">
                <div class="col-lg-9">
                    ...
                    <div class="card mt-4" ng-app="myapp" ng-controller="BusController">
                        <img class="card-img-top img-fluid" src="resources/image/{{pdo.busid}}.jpg" width="600" height="100" alt="">
                        <div class="card-body">
                            <h1 class="card-title">Car id : {{pdo.busid}}</h1>
                            <h2 class="card-title"> Car Name :  {{pdo.busname}} </h2>
                            <h2 class="card-title"> Station :  {{pdo.startplace}} </h2>
                            <h2 class="card-title"> Last station  :  {{pdo.endplace}} </h2>
                            
                            <h3>car Fare  : Tk.{{pdo.fare}}</h3>
                           
                                          
                        </div>
                        <div class="btn-group cart">
                            <c:if test="${sessionScope.UserLoggedIn !=null}">
                                <h1> <a href="showbusbookingpage?getPackId={{pdo.busid}}" 
                                        class="" id="cartbutton" style="padding: 10px; border: 3px solid red;"> Book Package </a></h1> 
                                        <h2><a href="busdescription" class="" id="cartbutton" style="padding: 10px;"> View More package </a></h2>
                            </c:if>
                            <c:if test="${sessionScope.UserLoggedIn == null}">
                                <a href="perlogcheck?getPackId={{pdo.busid}}"
                                   class="btn btn-primary" id="loginproductsbutton"
                                   >Book Package</a>
                                <a href="busdescription" class="btn btn-primary" id="viewproductsbutton">  View More Products </a>
                            </c:if>
                        </div>
                    </div>
                </div>
